"""Generate immutable Phase 1 experiment YAML files with calibrated epsilon."""
...
def main() -> None:
    ...
    parser.add_argument("--spec", type=Path, default=Path("configs/phase1_grid_spec.yaml"))
    parser.add_argument("--output", type=Path, default=Path("configs/generated/phase1"))
        ...
        for heterogeneity in spec["heterogeneity"]:
            for client_id in spec["deletion_client_ids"]:
                for seed in spec["seeds"]:
                    ...
                    config["data"]["heterogeneity"] = float(heterogeneity)
                    config["deletion"]["client_id"] = int(client_id)
                    config["privacy"] = privacy
                    config["experiment_name"] = f"{base['experiment_name']}_{epsilon_name}_h{heterogeneity:g}_delete{client_id}_seed{seed}"
                    name = f"{epsilon_name}_h{heterogeneity:g}_delete{client_id}_seed{seed}.yaml"
